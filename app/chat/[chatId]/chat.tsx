...
export default function ChatComponent({ chatData, isNewChat = false, resume = false }: { chatData: { id: string; messages: MyUIMessage[] }; isNewChat?: boolean; resume?: boolean }) {
  ...
  return (
    <div className="flex flex-col py-24 mx-auto w-full max-w-md stretch">
      {messages.map((message) => (
        <Message key={message.id} message={message} regenerate={regenerate} sendMessage={sendMessage} status={status} />
      ))}
      <ChatInput
        status={status}
        stop={() => {
