...
export default function Message({
  ...
  const date = message.metadata?.createdAt ? new Date(message.metadata.createdAt).toLocaleString() : "";
  ...
  return (
    ...
    >
      ...
      {message.role === "user" && (
        <>
          <button onClick={() => regenerate({ messageId: message.id })} className="px-3 py-1 mt-2 text-sm transition-colors bg-gray-200 rounded-md hover:bg-gray-300" disabled={status !== "ready"}>
            Regenerate
          </button>
          <button
