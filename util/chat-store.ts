...
export async function saveChat({ id, activeStreamId, messages, canceledAt }: { id: string; activeStreamId?: string | null; messages?: MyUIMessage[]; canceledAt?: number | null }): Promise<void> {
...
export async function appendMessageToChat({ id, message }: { id: string; message: MyUIMessage }): Promise<void> {
  const chat = await readChat(id);
